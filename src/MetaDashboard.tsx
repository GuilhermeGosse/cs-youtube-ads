...
export function MetaDashboard() {
  const [sortBy, setSortBy] = useState<
    "cost" | "reach" | "frequency" | "conversions" | "clicks" | "impressions"
  >("cost");
  const [searchTerm, setSearchTerm] = useState("");
  const [rawCampaignsData, setRawCampaignsData] = useState<
    DailyCampaignEntry[]
  >([]);
 ...
 useEffect(() => {
  ...
  const filteredDailyCampaigns = useMemo(() => {
    ...
    if (dateRange.start) {
      filtered = filtered.filter(
        (c) => new Date(c.date) >= new Date(dateRange.start)
      );
    }
    if (dateRange.end) {
  ...
  const aggregatedCampaigns = useMemo(() => {
    ...
    filteredDailyCampaigns.forEach((campaign) => {
      ...
      if (existing) {
        ...
        existing.reach = (existing.reach || 0) + (campaign.reach || 0);
    ...
    let aggregatedList: AggregatedCampaign[] = Array.from(
  ...
  const totals = useMemo(
    () =>
      aggregatedCampaigns.reduce(
        (acc, c) => ({
          clicks: acc.clicks + c.clicks,
          impressions: acc.impressions + c.impressions,
          reach: (acc.reach || 0) + (c.reach || 0),
          cost: acc.cost + c.cost,
          conversions: acc.conversions + c.conversions,
        }),
        { clicks: 0, impressions: 0, reach: 0, cost: 0, conversions: 0 }
      ),
    [aggregatedCampaigns]
  );

  const averageFrequency = useMemo(() => {
    return totals.reach && totals.reach > 0
  ...
  const MetricCard = ({
    ...
    <div
      className={`${gradient} rounded-xl p-6 shadow-lg transform hover:scale-105 transition-all duration-300`}
    >
      <div className="flex items-center justify-between">
        <div>
          <p className={`${textColor} text-sm font-medium opacity-90`}>
            {title}
          </p>
  ...
  const maxValues = useMemo(
    () => ({
      ...
      conversions: Math.max(
        ...aggregatedCampaigns.map((c) => c.conversions),
        0
      ),
      clicks: Math.max(...aggregatedCampaigns.map((c) => c.clicks), 0),
      impressions: Math.max(
        ...aggregatedCampaigns.map((c) => c.impressions),
        0
      ),
      frequency: Math.max(
        ...aggregatedCampaigns.map((c) => c.frequency || 0),
        0
      ),
    }),
    [aggregatedCampaigns]
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          ...
          <MetricCard
            ...
            subtitle="Exibições dos anúncios"
            icon={Eye}
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-gray-900 flex items-center">
              ...
              Resumo por Campanha (Total no Período)
            </h2>
            <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center justify-between mb-6 border-b pb-4">
            <h2 className="text-xl font-bold text-gray-900 flex items-center">
              {detailsView === "total" ? (
                ...
                <List className="mr-2 text-green-600" />
              )}
              {detailsView === "total"
                ? "Detalhes por Campanha (Total)"
                : "Detalhes Diários das Campanhas"}
            </h2>
            <div className="flex space-x-1 bg-gray-200 p-1 rounded-lg">
              <button
                onClick={() => setDetailsView("total")}
            ...
            {detailsView === "total" && (
              <div>
                {aggregatedCampaigns.map((c) => (
                  <div
                    key={c.id}
                    className="border border-gray-200 rounded-lg p-4 mb-4 hover:shadow-md transition-all duration-200"
                  >
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        <span className="text-gray-600">Total Inv.:</span>
                      ...
                      <div>
                        <span className="text-gray-600">Total Alcance:</span>
                      ...
                      <div>
                        <span className="text-gray-600">Total Conversões:</span>
                      ...
                      <div>
                        <span className="text-gray-600">Total Cliques:</span>
                      ...
                      <div>
                        ...
                        <span className="ml-2 font-medium">
                          {c.frequency ? c.frequency.toFixed(2) : "0.00"}
                        </span>
                      </div>
                      ...
                      <div>
                        ...
                        <span className="ml-2 font-medium">
                          {formatCurrency(c.costPerConversion)}
                        </span>
                      </div>
            ...
            {detailsView === "daily" && (
              <div>
                {filteredDailyCampaigns.map((c) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="font-semibold text-gray-900 text-sm truncate max-w-xs">
                        {c.name.replace("[CS] Youtube - ", "")}
                      </h3>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <span className="ml-2 font-medium">
                          {c.reach && c.reach > 0
                            ? (c.impressions / c.reach).toFixed(2)
                            : "0.00"}
                        </span>
                      </div>
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white rounded-xl shadow-lg p-6">
            <h2 className="text-xl font-bold text-green-600 mb-4">
              Top 5 - Melhor Custo por Conversão
            </h2>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            <h2 className="text-xl font-bold text-red-600 mb-4">
              Piores 5 - Maior Custo por Conversão
            </h2>
            <ResponsiveContainer width="100%" height={300}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="text-center p-4 bg-green-50 rounded-lg">
              <p className="text-3xl font-bold text-green-600">
                {formatCurrency(totals.cost / (totals.conversions || 1))}
              </p>
              <p className="text-gray-600 mt-2">Custo Médio por Conversão</p>
            </div>
            <div className="text-center p-4 bg-orange-50 rounded-lg">
              <p className="text-3xl font-bold text-orange-600">
                {formatCurrency(totals.cost / (totals.clicks || 1))}
              </p>
        ...
        <footer className="mt-12 text-center text-gray-500 text-sm">
          &copy; {new Date().getFullYear()} Canal Solar - Todos os direitos
          reservados.
        </footer>
      </div>
    </div>
